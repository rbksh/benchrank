...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: { name: string; value: number; color: string }[]; label?: string }) {
  ...
  return (
    <div className="bg-surface-2 border border-border rounded-md p-3 shadow-card-elevated text-xs font-mono-data">
      ...
      {payload.map((entry) => (
        <div key={`bar-tt-${entry.name}`} className="flex items-center justify-between gap-4 mb-1">
...
export default function ComparisonBarChartInner({ teams }: { teams: Team[] }) {
    ...
    <ResponsiveContainer width="100%" height={240}>
      <BarChart data={top8} margin={{ top: 8, right: 16, left: -10, bottom: 0 }} barCategoryGap="25%">
        ...
        <Legend
          ...
          iconSize={6}
        />
        <Bar dataKey="Speed" fill="var(--neon-cyan)" radius={[2, 2, 0, 0]} maxBarSize={14} />
